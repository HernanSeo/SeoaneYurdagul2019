...
q={'id' 't' 'b' 'kt' 'kn' 'zt' 'zn' 'c' 'ct' 'cn' 'yt' 'yn' 'pn' 'mu' 'kappa' 'omega' 'g' 'tax'}; 
...
for m=1:2
    clear t
    if m==1
        polf=csvread('\trend\data_sim.txt'); % with trend
    elseif m==2
        polf=csvread('\no_trend\data_sim.txt'); % without trend
    end
    r = 0.04;
    ...
    for j=1:nid
        ...
        y(:,j)=yt(:,j)+pn(:,j).*yn(:,j);
        rer(:,j) = (omega(:,j).^(1/(1+eta))+(1-omega(:,j)).^(1/(1+eta)).*pn(:,j).^(eta/(1+eta))).^((1+eta)/eta); % increase appreciation
