...
**** IF Solving Social Planner's problem, uncomment the line to run tax.do. That would creat the tax numbers in Table 4 ****
...
preserve
gen event=ssnew
noisily sum event if t>100
run "C:\Users\eyurdagu\Dropbox\project - overborr_missallocation\codes\outputs\before_after_SS_002.do"
restore
...
gen event=(mupos==1)
noisily sum event if t>100
run "C:\Users\eyurdagu\Dropbox\project - overborr_missallocation\codes\outputs\before_after_mupos.do"
restore


preserve
sum logg
gen event=(L1.logg>logg+2*r(sd))
noisily sum event if t>100
run "C:\Users\eyurdagu\Dropbox\project - overborr_missallocation\codes\outputs\before_after_gdrop.do"
restore


twoway (line bpr t, lcolor(red) lpattern(solid)) (line blim t, lcolor(green) lpattern(dash)) ///
(line mu t, lcolor(gs4) lpattern(tight_dot) yaxis(2)) if t>250 & t<350 & id==20, ///
legend(order(1 "b'" 2 "Borrowing limit" 3 "mu") rows(1)  region(lcolor(white)) size(medium)) graphregion(fcolor(white) lcolor(white)) plotregion(margin(large))
graph export "sim_bblimzoom.pdf", replace
